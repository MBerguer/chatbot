...
class DepositCurrencyConversation extends Conversation
{
        ...
        if ($acc)
        {
            ...
            if($amt>0)
            {
                ...
                else
                {
                    ...
                    $convAmt = ($amt * 2.1);
                    $newAmt = $oldAmt + $convAmt; // HARDCODED JUST FOR TESTING
                    $acc->update(array('amount' => $newAmt));
                }
            }
            $this->say('You have successfully deposited '.$acc->currency.' '.floatval($convAmt).' ('.$fixedCurr.' '.$amt.') to your account');
            $log = new Logs();
